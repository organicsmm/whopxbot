...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Grinzo should only credit the exact hosted-button amounts.
    ...
    if (trustedIntent) {
      ...
      if (!profileByEmail || profileByEmail.length === 0) {
        ...
        await notifyTelegram(
          supabase,
          `⚠️ <b>Grinzo — Razorpay payment, NO user match</b>\n` +
      ...
      if (profileByEmail.length > 1) {
        ...
        await notifyTelegram(
          supabase,
          `🚫 <b>Grinzo — Razorpay payment, ambiguous email</b>\n` +
          `Email: <code>${checkoutEmail}</code>\n` +
          `Amount: <b>₹${amountInr}</b>\n` +
          `Payment ID: <code>${paymentId}</code>\n` +
    ...
    if (userEmailFromNotes && profileEmail && userEmailFromNotes !== profileEmail) {
      ...
      await notifyTelegram(
        supabase,
        `🚫 <b>Grinzo — Blocked Razorpay Credit</b>\n` +
        `Amount: <b>₹${amountInr}</b>\n` +
        `Payment ID: <code>${paymentId}</code>\n` +
